...
本 PRD 的需求依据为公开资料桌面研究、竞品分析和对日常经营分析流程的小范围需求观察；不将其表述为正式访谈、问卷或已验证的用户事实。
...
上述数据反映的是 AI 使用和组织采用趋势，不直接证明运营人员的具体痛点；因此，以下内容仍以产品假设呈现，并在后续研究中验证。

| 产品假设 | 当前依据 | 后续验证方式 |
| --- | --- | --- |
| 业务人员更愿意用自然语言发起临时分析，而非先写 SQL。 | 桌面研究与对话式分析产品范式。 | 访谈观察首个任务路径、提问完成率和放弃点。 |
| 用户对结果可信度的判断依赖数据范围、工具结果和口径说明。 | 通用模型的数据核验需求与可信执行设计。 | 可用性测试比较“仅结论”与“结论+依据”的采纳率和可信度评分。 |
| 对高频固定指标，传统看板仍更高效。 | BI 产品范式分析。 | 访谈区分固定看数与临时探索任务占比。 |
| 个人知识库可减少重复解释业务口径。 | 指标、规则、背景知识与检索式复用的产品设计。 | 追踪口径被保存、检索、确认和实际复用的比例。 |
...
![图 1：数探主工作台，围绕同一会话呈现数据、对话、工具过程与结果](./images/11.png)

*图 1：主工作台。左侧承载数据、知识库和历史能力，中间承载当前分析会话。*
...
![图 4：对话式分析过程与结果呈现](./images/3.png)

*图 4：对话结果。结论、数据依据和后续追问在同一会话内连续呈现。*

![图 5：基于真实结果生成的分析图表](./images/4.png)

*图 5：图表结果。图表与对应分析结论共同呈现，便于复核与汇报。*
...
![图 6：个人数据知识库中的业务口径与背景知识](./images/6.png)

*图 6：数据知识库。用户可维护指标定义、业务规则和背景知识，供后续分析检索复用。*
...
![图 7：统一数据接入入口](./images/9.png)

*图 7：数据接入入口。用户可选择上传文件、连接数据库、在线表格或业务接口。*
...
![图 8：分析工具选择面板](./images/8.png)

*图 8：分析工具。显式工具选择与自然语言自动路由并行，满足探索与明确方法两类使用方式。*
...
![图 9：个人知识在对话中的引用](./images/5.png)

*图 9：知识引用。系统在相关任务中引用已确认的业务口径与背景知识。*

![图 10：历史分析与个人化能力](./images/12.png)
...
| 模型数据边界 | 完成回答所需提示词、schema 和相关数据由服务端发送至托管模型服务；部署方需按自身政策选择模型接入。 |
