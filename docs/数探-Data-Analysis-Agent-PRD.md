...
MVP 聚焦一条最短分析闭环：**文件上传与理解 → 自然语言提问 → 工具执行 → 查看依据 → 继续追问**。数据和口径充分时直接执行；关键歧义时确认；数值结论来自工具结果并展示依据。版本范围以这一闭环为起点，逐步扩展复用与组织能力。
...
数探不是“替用户猜答案”的聊天机器人，也不替代企业的固定经营看板。它面向文件起步的临时经营分析，通过真实数据执行、关键口径确认和可核验结果，降低业务人员完成一次分析的门槛。
...
| 可信目标 | 按“真实执行优先、过程可追溯、失败显式反馈”原则交付结果。 |
...
| 自动执行，而非逐步确认 | 频繁确认抬高使用门槛。 | 落实“自动执行优先、关键歧义确认”。 | 每次工具调用均要求用户批准。 | 减少中断；风险是模型误判充分性，需展示范围并允许更正。 |
| 展示工具轨迹，不展示完整推理 | 用户需要验证依据，不需要阅读不稳定的内部思维。 | 落实“过程可追溯、不暴露完整推理”。 | 展示完整模型内部推理过程。 | 可复核且可读；风险是摘要不足，需支持查看结构化结果产物。 |
| 建立个人知识库 | 指标定义和业务规则每次重复说明，易造成口径漂移。 | 保存指标、规则、背景知识及可检索文档；以用户私有范围检索。 | 只保存聊天记录。 | 知识库服务于口径复用，而非聊天归档。风险是过期知识影响分析，需要编辑、启停和确认机制。 |
| MVP 优先文件上传 | 企业数据源接入、权限与治理成本高。 | 先支持 CSV/Excel，验证最短分析闭环。 | 在 MVP 把数据库、API、在线表格和协作作为主路径。 | 文件成本低、范围广；风险是文件版本和多文件关联歧义，需要显式提示。 |
...
| P2：规模化扩展 | 团队知识库、协作权限、定时分析、主动异常发现与企业级治理。 | 先满足权限、稳定性和审计要求，再进入规模化使用。 |
...
![图 2：统一数据接入入口](./images/9.png)

*图 2：数据接入入口。用户可选择上传文件、连接 SQL 数据库、Google Sheets、业务 API 或使用示例数据。*

![图 3：上传 Excel 或 CSV 数据文件](./images/10.png)

*图 3：文件上传。用户在同一入口完成文件选择、上传进度查看和解析发起。*

![图 4：数据预览与 Sheet、字段选择](./images/2.png)

*图 4：数据预览。用户在分析前确认 Sheet、字段和样例数据。*
...
![图 5：分析工具选择面板](./images/8.png)

*图 5：分析工具。用户可显式选择数据理解、图表、建模、预测或导出能力，也可直接通过自然语言发起任务。*
...
![图 6：对话式分析过程与结果呈现](./images/3.png)

*图 6：对话结果。结论、数据依据和后续追问在同一会话内连续呈现。*

![图 7：基于真实结果生成的分析图表](./images/4.png)

*图 7：图表结果。图表与对应分析结论共同呈现，便于复核与汇报。*
...
![图 8：个人数据知识库中的业务口径与背景知识](./images/6.png)

*图 8：数据知识库。用户可维护指标定义、业务规则和背景知识，供后续分析检索复用。*

![图 9：个人知识在对话中的引用](./images/11.png)

*图 9：知识库引用。系统基于已沉淀的业务口径回答问题，并在口径缺失时引导用户补充定义。*
...
| 体验与额度 | 默认额度为未登录用户每日 5 次分析、登录用户每日 30 次分析；部署者可通过服务端配置调整。 | 同一身份同时仅运行 1 个分析任务；额度按自然日重置，登录后当天体验用量并入账户额度。 |
...
![图 10：个人偏好与个性化设置](./images/12.png)

*图 10：个人偏好。用户可维护稳定的输出偏好，使后续会话保持一致。*

## 7.8 FR-08 团队协作与自动化能力（P2）
...
失败处理遵循“真实执行优先、失败显式反馈”原则，明确用户可见的状态、恢复方式与结果保留规则。
...
指标体系用于定义上线或可用性测试后的观测口径；实际数值需同时记录样本范围与统计周期。
...
离线评测在固定数据、模型配置和案例下运行真实工具调用，用于比较 Prompt A/B 对工具路由、SQL、图表、回答安全性和效率的影响。
...
以下数据来自 `evals/reports/prompt_ab_test_report.md` 的 20 案例独立测试，`repeat=1`。

| 指标 | Prompt B 相对 Prompt A 的变化 |
| --- | --- |
| 工具路由正确率 | 保持不变。 |
| SQL 执行成功率 | 保持不变。 |
| SQL 结果正确率 | 提升 6.25 个百分点。 |
| 端到端任务完成率 | 提升 5.00 个百分点。 |
| 幻觉代理指标 | 保持不变。 |
| 图表合适率 | 保持 100.00%。 |
| 平均 Token | 增加 963.10（约 5.04%）。 |
| 平均延迟 | 缩短 5508.50 ms（约 24.23%）。 |
| 平均成本 | 未配置价格，不估算。 |

独立测试中 Prompt B 改善 1 个案例、回归 0 个案例。开发集 Prompt A 的 35 次结果显示主要失败集中在 SQL 结果错误（16）、图表类型错误（3）、遗漏必需工具（3）和不支持的因果主张（3）；后续迭代优先优化结果校验与任务约束。
...
| 主题 | 要求 |
| --- | --- |
| 可用性 | 网络、模型和工具不可用时返回可区分的失败状态；通过可用性、任务完成率和错误类型持续观测服务质量。 |
